...
class Advisor {
  ...
  handleSyncChange() {
    if (Preferences.isSet('services.sync.username')) {
      this.panel.port.emit('syncEnabled');
    } else {
      this.panel.port.emit('syncDisabled');
    }
  }
  ...
  createPasswordChangeListener() {
    this.panel.port.on('passwordChange', () => {
      this.telemetryLog.passwordChange(this.activeRecDomain);
    });
  }

  createNoAccountListener() {
    this.panel.port.on('noAccount', () => {
      this.telemetryLog.noAccount(this.activeRecDomain);
    });
  }
  ...
  createSignupListener() {
    this.panel.port.on('signup', () => {
      ...
      this.telemetryLog.signup(this.activeRecDomain);
    });
  }

  createNoSignupListener() {
    this.panel.port.on('noSignup', () => {
      this.telemetryLog.noSignup(this.activeRecDomain);
    });
  }
  ...
  createDisableForeverListener() {
    this.panel.port.on('disableForever', () => {
      this.telemetryLog.disableAll(this.activeRecDomain);
      this.panel.hide();
      const win = tabs.activeTab.window;
      const button = this.getButton(win);
      button.hide();
